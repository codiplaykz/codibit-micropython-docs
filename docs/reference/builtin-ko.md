...
|--------|------|-------------|---------------|
| `is_pressed()` | 이벤트 감지 (눌림 순간) | - | 단일 이벤트 (메뉴 선택, 트리거 동작) |
| `is_holding()` | 현재 눌림 상태 확인 | - | 연속적인 동작 (버튼을 계속 누르고 있을 때) |
| `is_released()` | 이벤트 감지 (떼어짐 순간) | - | 단일 이벤트 (떼어짐 트리거) |
...
REPL stdin/stdout을 사용한 시리얼 통신 API입니다. ESP32 MicroPython에서 microbit과 유사한 시리얼 통신 인터페이스를 제공합니다. `sys.stdin`과 `sys.stdout`을 사용하여 클라이언트(웹앱 등)와 사용자 코드 간 통신을 지원합니다. [2-way handshake 프로토콜](../explanation/serial-protocol-ko.md#2-way-handshake-프로토콜)을 사용하여 클라이언트와의 연결을 자동으로 수립합니다.
...
**매개변수:**
- `data` (str or bytes): 전송할 문자열 또는 바이트. 바이트는 자동으로 UTF-8 문자열로 변환됩니다.

**반환값:**
- `int`: 전송된 바이트 수. 오류 시 0을 반환합니다.
...
**주의사항:**
- 바이트는 자동으로 UTF-8 문자열로 변환됩니다
- `flush()` 호출로 즉시 전송을 보장합니다
- 예외 발생 시 0을 반환합니다 (조용한 실패)

#### `serial.read()`

시리얼 입력에서 사용 가능한 데이터를 읽습니다 (비차단).

사용 가능한 모든 데이터를 즉시 반환합니다. 데이터가 없으면 빈 문자열을 반환합니다. ENQ(0x05)를 감지하면 자동으로 ACK(0x06)를 응답합니다. 제어 문자(ENQ/ACK)는 사용자 데이터에서 필터링됩니다.

**반환값:**
- `str`: 읽은 문자열. 데이터가 없으면 빈 문자열을 반환합니다.
...
**주의사항:**
- 비차단: 데이터가 없어도 즉시 반환합니다
- 연결 수립: 내부적으로 [2-way handshake 프로토콜](../explanation/serial-protocol-ko.md#2-way-handshake-프로토콜)을 자동 처리합니다
- 제어 문자: ENQ/ACK는 필터링되어 사용자 코드에 노출되지 않습니다
- 빈 문자열: 데이터가 없을 때 빈 문자열을 반환합니다

#### `serial.write_line(data)`

시리얼 출력으로 문자열과 줄바꿈 문자(`\n`)를 씁니다.
...
**주의사항:**
- 데이터 뒤에 자동으로 `\n`을 추가합니다
- 라인 기반 통신 프로토콜에 유용합니다

#### `serial.read_line()`

시리얼 입력에서 라인을 읽습니다 (비차단, `\n`까지).

줄바꿈 문자(`\n` 또는 `\r`)를 받을 때까지 버퍼에 데이터를 축적합니다. 완전한 라인이 없으면 빈 문자열을 반환합니다. ENQ(0x05)를 감지하면 자동으로 ACK(0x06)를 응답합니다. 제어 문자(ENQ/ACK)는 사용자 데이터에서 필터링됩니다.

**반환값:**
- `str`: 읽은 라인 문자열 (줄바꿈 제외). 완전한 라인이 없으면 빈 문자열을 반환합니다.
...
**주의사항:**
- 비차단: 완전한 라인이 없어도 즉시 반환합니다
- 버퍼 관리: 줄바꿈을 받을 때까지 데이터를 축적합니다
- 연결 수립: 내부적으로 [2-way handshake 프로토콜](../explanation/serial-protocol-ko.md#2-way-handshake-프로토콜)을 자동 처리합니다
- 제어 문자: ENQ/ACK는 필터링되어 사용자 코드에 노출되지 않습니다
- 빈 문자열: 완전한 라인이 없을 때 빈 문자열을 반환합니다

### 연결 수립

Serial 클래스는 2-way handshake 프로토콜을 사용하여 연결 수립을 자동으로 처리합니다:

1. **클라이언트가 ENQ (0x05) 전송**: 연결 요청 (주기적)
2. **보드가 ACK (0x06) 응답**: 준비 완료 응답
3. **연결 수립**: 데이터 전송 시작 가능

**주요 특징:**
- **자동**: 사용자 코드에서 명시적 연결 호출 불필요
- **비동기**: 어느 쪽이 먼저 시작되어도 동작
- **투명**: 연결 수립은 내부적으로 처리됨
- **비침투적**: 제어 문자는 사용자 데이터에서 필터링됨

프로토콜 설계 및 구현 상세는 [시리얼 통신 프로토콜](../explanation/serial-protocol-ko.md)을 참고하세요.

### 하드웨어 정보

- **인터페이스**: REPL stdin/stdout (`sys.stdin`, `sys.stdout`)
- **프로토콜**: 2-way handshake (ENQ/ACK)
- **보드레이트**: 115200 (ESP32 MicroPython REPL 기본값)
- **데이터 형식**: 일반 텍스트 문자열
- **제어 문자**: ENQ (0x05), ACK (0x06) - 사용자 데이터에서 필터링됨

### 주의사항

1. **비차단**: 모든 읽기 작업은 비차단이며 즉시 반환합니다
2. **연결 수립**: 내부적으로 자동 처리되며 사용자 코드 불필요
3. **제어 문자**: ENQ/ACK는 필터링되어 사용자 코드에 노출되지 않습니다
4. **버퍼 관리**: `read_line()`은 줄바꿈까지 데이터를 축적하기 위해 내부 버퍼를 사용합니다
5. **에러 처리**: 오류 발생 시 빈 문자열 또는 0을 반환합니다 (조용한 실패)
6. **메모리 효율성**: MicroPython 메모리 제약을 고려하여 설계되었습니다
7. **클라이언트 호환성**: 웹앱 및 기타 시리얼 클라이언트와 작동합니다
8. **비동기 연결**: 어느 쪽이 먼저 시작되어도 정상 동작합니다

자세한 사용 예제와 패턴은 [시리얼 통신 사용하기](../how-to/serial-usage-ko.md)를 참고하세요. 프로토콜 설계 상세는 [시리얼 통신 프로토콜](../explanation/serial-protocol-ko.md)을 참고하세요.