...
3. **상태 문자열**: `get_status()` 메서드는 5단계 상태 문자열("very_quiet", "quiet", "medium", "loud", "very_loud")을 반환합니다
4. **이벤트 감지**: 센서는 주변 소음에 자동으로 보정되고 변화를 감지합니다
5. **임계값 확인**: `is_loud()`나 `is_quiet()` 같은 메서드는 미리 정의된 임계값을 사용합니다
